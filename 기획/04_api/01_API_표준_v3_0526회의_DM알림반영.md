...
| 항목     | Mock                       | Real (목표)                                           |
| ------ | -------------------------- | --------------------------------------------------- |
| 응답 래퍼  | `{success, data, message}` | `{success, message, data, error?: {code, message}}` |
...
성공 응답은 `message` 를 포함합니다. `error` 필드는 성공 응답에 포함하지 않습니다 (`@JsonInclude(NON_NULL)`).
...
- 동사 금지. 단 명확한 액션성 서브리소스는 PATCH 허용:
  ...
  - `PATCH /api/files/download-requests/{id}/approve`
...
>
> 5/26 TA 전체 회의 이후 요청 게시판에는 `reject`, `reopen`, `cancel` 흐름을 두지 않는다.
> 상태는 `REQUESTED → IN_PROGRESS → COMPLETED` 로만 간다.
>
> 에셋 파일 다운로드는 별도 다운로드 승인 요청 흐름을 둔다.
> 요청자가 `POST /api/files/{fileId}/download-requests` 로 승인 요청을 보내고,
> 제작자가 `PATCH /api/files/download-requests/{id}/approve` 로 승인하면 다운로드가 가능하다.
>
> 요청 게시판 상태 변경은 DM 시스템 알림으로 사용자에게 전달한다.
> `PATCH /api/requests/{id}/assign` 성공 시 요청자에게 “{assigneeNickname}님이 요청을 수락하여 제작중입니다.” 알림 DM을 보낸다.
> `POST /api/posts`에서 `linkedRequestId`로 요청이 완료될 때 요청자에게 “{assigneeNickname}님이 요청하신 에셋 제작을 완료했습니다.” 알림 DM을 보낸다.
> 시스템 알림 DM은 일반 사용자가 직접 보내는 메시지가 아니라 서버가 자동 생성한다.
